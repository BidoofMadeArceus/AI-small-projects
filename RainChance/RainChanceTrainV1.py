from math import e

def train(X1_test,X2_test,weight1,weigh2,bias,learn_rate):
  for i in range(10000):
  W1_errors = []
  W2_errors = []
  b_errors
  for i in range(len(X1_test):
    z = b + (weight_1 * X1_test[i]) + (weight_2 * X2_test[i])
    predict = 1 / (1 + e**(0-z)
    w1_error = (predict - y[i]) * X1_test[i]
    w2_error = (predict - y[i]) * X2_test[i]
    b_error = predict - y[i]
    W1_errors.append(w1_error)
    W2_errors.append(w2_error)
    b_errors.append(b_error)
  W1slope = sum(W1_errors) / len(X1_test)
  W2slope = sum(W2_errors) / len(X1_test)
  Bslope = sum(b_errors) / len(X1_test)
  weight_1 -= (learn_rate * W1slope)
  weight_2 -= (learn_rate * W2slope)
  bias -= (learn_rate * Bslope)
